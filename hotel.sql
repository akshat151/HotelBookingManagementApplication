...
insert  into `login`(`Username`,`Password`) values ('Admin','Manish');
...
AFTER DELETE ON status_current
FOR EACH ROW
  UPDATE Room 
     SET status = 0
...
AFTER DELETE ON status_advance
FOR EACH ROW
  UPDATE Room 
...
status1 varchar(1);
BEGIN
SELECT INTO status1 FROM Room WHERE Roomno=New.Roomno;
if(status1='1')
THEN RAISE_APPLICATION_LAYER(-20001,'Room Already BOOKED');
END IF;
END//
